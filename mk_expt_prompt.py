...
def getOpts(expt_name):
    opt = getDefaultOpts()
    print(expt_name)
    if expt_name == "prompt_consistency_db":
        ...
        opt["instructions_file"] = "./assets/instructions_prompt.html"  # instructions appear at the beginning of the HIT
        opt[
            "short_instructions_file"
        ] = "./assets/short_instructions_prompt.html"  # short instructions are shown at the top of every trial
        opt["use_vigilance"] = False
        opt["paired"] = True
        opt["filename"] = lambda x: "{:05d}.jpg".format(x)  # may need to change
        opt["practice_filename"] = lambda x: "{:05d}.jpg".format(x)  # may need to change
        opt["practice_prompt_file"] = "assets/images/practice/prompts.json"
        opt["prompt_file"] = "assets/images/algos/prompts.json"  # a json file: dict: index -> caption
    else:
...
def mk_expt(expt_name):
    ...
    # check parameters
    checkOpts(opt)
    all_prompts = json.load(open(str(opt["prompt_file"]), 'r'))
    practice_prompts = json.load(open(str(opt["practice_prompt_file"]), 'r'))
    ...
    for i in range(opt["Npairs"]):
        ...
        head_prompts.append("prompts%d" % i)
    ...
    pickle.dump([which_ind0, which_ind1], open(os.path.join(opt["expt_name"], "indices.pkl"), "wb"))
    ...
    for nn, data in enumerate(zip(which_alg, which_ind0, which_ind1, which_side, vigilance)):
        ...
        if nn in practice_indices:
            ...
            if cur_which_side == 0:
                gt_side.append("left")
                print(
                    ("%s/" + opt["practice_filename"](cur_which_ind0 % opt["practice_Nimgs"])) % opt["practice_gt_path"]
                )
                images_left.append(
                    ("%s/" + opt["practice_filename"](cur_which_ind0 % opt["practice_Nimgs"])) % opt["practice_gt_path"]
                )
                images_right.append(
                    ("%s/" + opt["practice_filename"](cur_which_ind1 % opt["practice_Nimgs"])) % cur_alg_name
                )
                temp_prompt = practice_prompts["{:05d}".format(cur_which_ind0 % opt["practice_Nimgs"])]
                prompts.append(f'"{temp_prompt}"')
            else:
                gt_side.append("right")
                images_left.append(
                    ("%s/" + opt["practice_filename"](cur_which_ind0 % opt["practice_Nimgs"])) % cur_alg_name
                )
                images_right.append(
                    ("%s/" + opt["practice_filename"](cur_which_ind1 % opt["practice_Nimgs"])) % opt["practice_gt_path"]
                )
                temp_prompt = practice_prompts["{:05d}".format(cur_which_ind0 % opt["practice_Nimgs"])]
                prompts.append(f'"{temp_prompt}"')
        else:
            ...
            if cur_which_side == 0:
                ...
                temp_prompt = all_prompts["{:05d}".format(cur_which_ind0)]
                prompts.append(f'"{temp_prompt}"')
            else:
                gt_side.append("right")
    ...
    prompts = np.array(prompts).reshape((H, N))

    head_gt_side = np.array(head_gt_side)
    head_images_left = np.array(head_images_left)
    head_images_right = np.array(head_images_right)
    head_prompts = np.array(head_prompts)

    csv_out = np.concatenate(
        (
            np.concatenate((head_gt_side, head_images_left, head_images_right, head_prompts), axis=1),
            np.concatenate((gt_side, images_left, images_right, prompts), axis=1),
        ),
        axis=0,
    )
    ...
    # html code generator
    # embed()
    html = fileread("assets/index_template_prompt.html", breakcode=breakcode)
    ...
    s = (" ").join(
        [
            'sequence_helper("${gt_side%d}","${images_left%d}","${images_right%d}","${prompts%d}");\n' % (i, i, i, i)
            for i in range(opt["Npairs"])
        ]
    )
...
parser.add_argument("-n", "--name", type=str, default="prompt_consistency_db")
